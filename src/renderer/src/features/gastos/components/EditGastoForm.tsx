...
interface Props { userId: number; expense: Expense; onClose: () => void }
...
export function EditGastoForm({ userId, expense, onClose }: Props) {
  ...
  function onSubmit(data: GastoFormValues) {
    updateExpense({
      ...
      onSuccess: () => { toast.success("Gasto atualizado!"); onClose() },
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" noValidate>
        ...
        <FormField control={form.control} name="isPaid" render={({ field }) => (
          <FormItem className="flex-row items-center gap-2 space-y-0">
            <FormControl>
              <input type="checkbox" id="editIsPaid" checked={field.value} onChange={field.onChange}
                className="size-4 rounded border-input accent-primary cursor-pointer" />
            </FormControl>
            <FormLabel htmlFor="editIsPaid" className="cursor-pointer font-normal">Marcar como pago</FormLabel>
          </FormItem>
        )} />
